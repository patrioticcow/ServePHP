...
var domain = 'https://codemirror.massinflux.com?data=';
...
getResults.onclick = function () {
  ...
  var toRun = domain + encodeURIComponent(editor.getValue()).replace(/%20/g, '+');
  loadXMLDoc(response, toRun);
}

var initToRun = domain + encodeURIComponent(myTextArea.value).replace(/%20/g, '+');
loadXMLDoc(response, initToRun)
